...
public class UnifyPayQuery {
    public String createMessage(Mch mch,  Order order ) throws Exception{
        ...
        valueMap.put("service", Constrants.UNIFYPAYQUERY_SERVICE);
    ...
    public String payQuery(Mch mch, Order order) throws Exception{
        String xmlStringQuery = this.createMessage(mch, order);
        ...
        while(true) {
            ...
            if(returnMap.isEmpty() || (!returnMap.containsKey("trade_state"))){
                Thread.sleep(1000);
                map.put("times",map.get("times")+1);
                if(map.get("times") >= 100) {
                    break;
                }
                continue;
            }
            String state = returnMap.get("trade_state");
